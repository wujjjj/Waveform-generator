...
void LCD_ShowChar(u8 Xpos, u16 Ypos, u8 in_char){
	u32 index = 0, i = 0 , j = 0;
	u8 Xaddress = 0;
	...
	for(index = 0; index < 24; index++)
	{
		...
		for(j = 0;j < 3;j++){
			for(i = 0; i < 8; i++)
			{
				if(((zi[in_char][index * 3 + j] & (1 << i) )== 0x00) & (in_char != 250))
				{
					LCD_WriteRAM(TextColor);
				}
				else
...
void LCD_ShowString(u8 Line, u16 Column, u8 *ptr){
	u32 i = 0;
	u16 refcolumn = Column;
